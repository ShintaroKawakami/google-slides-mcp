...
- **何を**: 短い再開・状況変化は現在地と残りを1行。図解が必要な時はL1 ASCII。**説明HTMLは薄いストーリー＋左ナビ。細部詰め込み禁止。MermaidはPC横/スマホ縦。共有は0.0.0.0+Tailscale URL。Stitch不要**（本番UIだけStitch）。手順: `skills/visual-companion/references/explain-html-codex.md`
...
<!-- [2026-09-22][feat] 承認済みの会話改善①。
背景: 続きを探さず分かるようにする。確認済みと未確認を分け、全体を見直す時の図解義務は保つ。
他案不採用理由: 毎ターンの進捗再掲や、短い再開報告への大きな地図の強制は情報を増やす。
対応: 現在地の正本に1行の例外を置き、別スキルへ重複させない。
-->
- 再開時、状況が変わった時、短い進捗質問への返答では、最初の1行で「確認済みの現在地＋残り」を伝える。未確認のことは完了に含めない。同じ進捗を毎ターン再掲しない。
- 同じ作業の続きだけを伝える短い報告は、この1行でよい。この場合は §1-bis②⑦・§2-bis より本項を優先する。複数案件・依存関係の説明、選択肢の比較、全体の見直しには既存の図解を使う。
- 例：「修正は反映済み。残りはiPhoneでの接続確認です。」実際に確認できた状態だけで書く。
