...
await describe('google-slides-mcp HTTP server', async () => {
  await it('GET /health answers 200 without auth', async () => {
    const { status, body } = await request('/health');
    assert.equal(status, 200);
    assert.deepEqual(body, { ok: true, name: 'google-slides-mcp', version: '0.1.0' });
  });

  await it('GET on every OAuth discovery path answers empty 200 before auth', async () => {
    await Promise.all(
      OAUTH_DISCOVERY_GET_PATHS.map(async (path) => {
  ...
  await it('POST /register answers empty 200 before auth', async () => {
    const { status, body } = await request('/register', { method: 'POST' });
    assert.equal(status, 200);
    assert.deepEqual(body, {});
  });

  await it('GET / answers service info without auth', async () => {
  ...
  await it('POST /mcp without an api key answers 401', async () => {
  ...
  await it('POST /mcp with a matching ?api_key answers tools/list 200', async () => {
  ...
  await it('unknown paths answer 404', async () => {
